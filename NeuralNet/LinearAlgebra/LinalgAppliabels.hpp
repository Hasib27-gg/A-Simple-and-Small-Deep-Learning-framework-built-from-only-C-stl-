...
public:
  ...
  inline void operator()(NumberType &val)const noexcept {
...
public:
  inline void setValue(const NumberType c) { self.constant = c; }
  inline void operator()(NumberType &val)const noexcept { val -= self.constant; }
};
...
private:
public:
  inline void operator()(NumberType &val)const noexcept { val *= val; }
};
template <typename NumberType> class Cube {

public:
  inline void operator()(NumberType &val)const noexcept { val *= val * val; }
};
template <typename NumberType> class Exp {

public:
  inline void operator()(NumberType &val)const noexcept { val = std::exp(val); }
};
template <typename NumberType> class NaturalLog {

public:
  inline void operator()(NumberType &val)const noexcept { val = std::log(val); }
};
...
public:
  inline void operator()(NumberType &val)const noexcept { val = std::log10(val); }
};
...
public:
  ...
  inline void operator()(NumberType &val)const noexcept { val = std::log(base, val); }
};
template <typename NumberType> class Abs {

public:
  inline void operator()(NumberType &val)const noexcept { val = std::abs(val); }
};
template <typename NumberType> class Flip {

public:
  inline void operator()(NumberType &val)const noexcept { val = val * -1; }
};
template <typename NumberType> class Inverse {

public:
  inline void operator()(NumberType &val)const noexcept { val = (1 / val); }
};
...
public:
  ...
  inline void operator()(NumberType &val)const noexcept { val = std::max(self.key, val); }
};
...
public:
  inline void setValue(const NumberType key) { self.key = key; }
  inline void operator()(NumberType &val)const noexcept { val = std::min(self.key, val); }
};
...
public:
  inline void setValue(const NumberType key) { self.key = key; }
  inline void operator()(NumberType &val)const noexcept { val = val > key; }
};
...
public:
  inline void operator()(NumberType &val)const noexcept { val = std::sin(val); }
};
template <typename NumberType> class CoSine {
public:
  inline void operator()(NumberType &val)const noexcept { val = std::cos(val); }
};
template <typename NumberType> class Tangent {
public:
  inline void operator()(NumberType &val)const noexcept { val = std::tan(val); }
};
template <typename NumberType> class Secant {
public:
  inline void operator()(NumberType &val)const noexcept { val = (1 / std::cos(val)); }
};
template <typename NumberType> class CoSecant {
public:
  inline void operator()(NumberType &val)const noexcept { val = (1 / std::sin(val)); }
};
template <typename NumberType> class CoTangent {
public:
  inline void operator()(NumberType &val)const noexcept { val = (1 / std::tan(val)); }
};

template <typename NumberType> class ArcSine {
public:
  inline void operator()(NumberType &val)const noexcept { val = std::asin(val); }
};
template <typename NumberType> class ArcCoSine {
public:
  inline void operator()(NumberType &val)const noexcept { val = std::acos(val); }
};
template <typename NumberType> class ArcTangent {
public:
  inline void operator()(NumberType &val)const noexcept { val = std::atan(val); }
};
template <typename NumberType> class ArcSecant {
public:
  inline void operator()(NumberType &val)const noexcept { val = std::acos(1 / val); }
};
template <typename NumberType> class ArcCoSecant {
public:
  inline void operator()(NumberType &val)const noexcept { val = std::asin(1 / val); }
};
template <typename NumberType> class ArcCoTangent {
public:
  inline void operator()(NumberType &val)const noexcept { val = std::atan(1 / val); }
};

template <typename NumberType> class SineH {
public:
  inline void operator()(NumberType &val)const noexcept { val = std::sinh(val); }
};
template <typename NumberType> class CoSineH {
public:
  inline void operator()(NumberType &val)const noexcept { val = std::cosh(val); }
};
template <typename NumberType> class TangentH {
public:
  inline void operator()(NumberType &val)const noexcept { val = std::tanh(val); }
};
template <typename NumberType> class SecantH {
public:
  inline void operator()(NumberType &val)const noexcept { val = (1 / std::cosh(val)); }
};
template <typename NumberType> class CoSecantH {
public:
  inline void operator()(NumberType &val)const noexcept { val = (1 / std::sinh(val)); }
};
template <typename NumberType> class CoTangentH {
public:
  inline void operator()(NumberType &val)const noexcept { val = (1 / std::tanh(val)); }
};

template <typename NumberType> class ArcSineH {
public:
  inline void operator()(NumberType &val)const noexcept { val = std::asinh(val); }
};
template <typename NumberType> class ArcCoSineH {
public:
  inline void operator()(NumberType &val)const noexcept { val = std::acosh(val); }
};
template <typename NumberType> class ArcTangentH {
public:
  inline void operator()(NumberType &val)const noexcept { val = std::atanh(val); }
};
template <typename NumberType> class ArcSecantH {
public:
  inline void operator()(NumberType &val)const noexcept { val = std::acosh(1 / val); }
};
template <typename NumberType> class ArcCoSecantH {
public:
  inline void operator()(NumberType &val)const noexcept { val = std::asinh(1 / val); }
};
template <typename NumberType> class ArcCoTangentH {
public:
  inline void operator()(NumberType &val)const noexcept { val = std::atanh(1 / val); }
};
...
public:
  ...
  inline void operator()(NumberType &val) const noexcept { val = std::pow(val, self.pow); }
};
template <typename NumberType> class SquareRoot {
public:
  inline void operator()(NumberType &val) const noexcept { val = std::sqrt(val); }
};
template <typename NumberType> class CubeRoot {
public:
  inline void operator()(NumberType &val)const noexcept { val = std::cbrt(val); }
};

template <typename NumberType> class Round {
public:
  inline void operator()(NumberType &val)const noexcept { val = std::round(val); }
};

template <typename NumberType> class Ceiling {
public:
  inline void operator()(NumberType &val)const noexcept { val = std::ceil(val); }
};

template <typename NumberType> class Floor {
public:
  inline void operator()(NumberType &val)const noexcept { val = std::floor(val); }
};
template <typename NumberType> class Sign {
public:
  inline void operator()(NumberType &val)const noexcept { val = (val > 0) - (val < 0); }
};
