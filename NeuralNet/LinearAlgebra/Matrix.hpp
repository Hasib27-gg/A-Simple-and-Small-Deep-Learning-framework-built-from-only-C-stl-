...
inline void copy_if_allocated(
    ...
    std::memcpy(to_data, src, size * sizeof(NumberType));
...
template<typename NumberType1, 
 ...
 void zip_with_accum(
    ...
    for (std::size_t idx = 0; idx < size; idx++)
        res_data[idx] += func(m1_data[idx], m2_data[idx]);
}
...
template<typename NumberType1, 
 ...
 void zip_with_accum_static(
    ...
    for (std::size_t idx = 0; idx < bound; idx += 8)
    {
        const NumberType1* a = &m1_data[idx];
        const NumberType2* b = &m2_data[idx];
        NumberType3* c = &res_data[idx];
                ...
                for (size_t j_tile = 0; j_tile < m2.getColumnSize(); j_tile += col_tile) {
                    ...
                    for (size_t i = i_tile; i < i_end; i++) {
                        for (size_t k = k_tile; k < k_end; k++) {
                            ...
                            const NumberTypeB* b = &m2_data[k * m2.getColumnSize() + j_tile];
                            NumberTypeC* c       = &res_data[i * res.getColumnSize() + j_tile];

                            #define CST static_cast<NumberTypeC>

                            #pragma omp simd
                            for (std::size_t j = j_tile; j < j_end; j++) {
                                *c += a * CST((*b));
                                c++;
                                b++;
                            } // j
