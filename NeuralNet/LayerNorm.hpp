...
private:
  ...
  class _SCALED_DIV {
  public:
    std::size_t _hidden_size;
    inline NumTy3 operator()(const NumTy1 a, const NumTy2 b) const noexcept {
      return static_cast<NumTy3>(a) /
             (static_cast<NumTy3>(b * self._hidden_size));
    }
  };
...
public:
  ...
  init(const Linalg::NumPair &inp_shape, GammaInitTy &gamma_init = _get1,
    ...
    self._mean_res_buff =
        Linalg::createMatrix<InputType>(inp_shape.first, 1, 1);
    self._std_res_buff =
        Linalg::createMatrix<InputType>(inp_shape.first, 1, 1);
    ...
    self._gamma = Linalg::createMatrix<GammaType>(inp_shape.first, 1, 1);
    self._gamma_grad =
        Linalg::createMatrix<GammaGradType>(inp_shape.first, 1, 1);

    self._beta = Linalg::createMatrix<BetaType>(inp_shape.first, 1, 1);
    self._beta_grad =
        Linalg::createMatrix<BetaGradType>(inp_shape.first, 1, 1);
    ...
    _grad_scaler.setValue(inp_shape.second);
    self._scaled_div._hidden_size = inp_shape.second;
  }
  ...
  Linalg::Matrix<OutputGradType> backward(Linalg::Matrix<InputGradType> &grad) {
    ...
    Linalg::reduce_with_accum(grad, self._beta_grad, self._func_sum_2, 1);
  ...
  template <typename OptimizerType> inline void step(OptimizerType &optimizer) {
    optimizer.update(
      std::pair<Linalg::Matrix<GammaType> &, Linalg::Matrix<GammaGradType> &>(self._gamma, self._gamma_grad), 
      self._gamma_additionals);
    optimizer.update(std::pair<Linalg::Matrix<BetaType> &, Linalg::Matrix<BetaGradType> &>(self._beta, self._beta_grad), 
