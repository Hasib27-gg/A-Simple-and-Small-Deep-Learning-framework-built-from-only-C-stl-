...
namespace Linalg {
    ...
    std::size_t temp = _IdxPointer;
    std::size_t& idx = _IdxPointer;
    #ifndef NO_STEP_VALIDITY_CHECK
        if (flat_stop <= idx)
            throw std::invalid_argument("Stop must be ahead of current iterator position.");
    #endif
        for (;  idx + 7*step <= flat_stop ;  idx += 8*step)
    {
        ...
        for (; idx <= flat_stop; idx += step)
            func(data_pointer[idx]);
        
        if(returnBack) idx = temp;
    ...
    std::size_t flat_step = step.first*_ColSize + step.second;
        ...
        if (flat_stop <= idx)
            throw std::invalid_argument("Stop must be ahead of current iterator position.");
    #endif
        for (; idx + 7 * flat_step <= flat_stop; idx += 8 * flat_step)
        {
            func(data_pointer[idx]);
        ...
        for (; idx <= flat_stop; idx += flat_step)
            func(data_pointer[idx]);
